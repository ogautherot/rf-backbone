/**
 * ROM API from NXP for LPC11U6x
 */

#ifndef __ROM_API_H_INCLUDED__
#define __ROM_API_H_INCLUDED__
...
#define LPC_ROM_API (*(LPC_ROM_API_T **)ROM_DRIVER_BASE_LOC)
...
{
    void (*set_pll)(const uint32_t cmd[], uint32_t resp[]);   /*!< Set PLL function */
    void (*set_power)(const uint32_t cmd[], uint32_t resp[]); /*!< Set power function */
} PWRD_API_T;

/* **************************************************************************
 *                              Divisions
 * *********************************************************************** */
typedef struct IDIV_RETURN
{
    int quot; /*!< Quotient */
    int rem;  /*!< Remainder */
} IDIV_RETURN_T;

typedef struct UIDIV_RETURN
{
    unsigned quot; /*!< Quotient */
    unsigned rem;  /*!< Reminder */
} UIDIV_RETURN_T;

typedef struct ROM_DIV_API
{
...
typedef CALLBK_T UART_CALLBK_T;
...
typedef void (*IAP)(uint32_t cmd[], uint32_t param[]);
#define IAP_ENTRY ((IAP)IAP_LOCATION)

extern unsigned int command_param[5];
extern unsigned int status_result[5];

#endif // __ROM_API_H_INCLUDED__
